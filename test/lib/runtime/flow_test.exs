defmodule ActiveMonitoring.Runtime.FlowTest do
  ...
  describe "forwarding call on all symptoms" do
    ...
    test "it should forward the call on all symptoms positive", %{campaign: campaign, call: call} do
      ...
      assert %Call{current_step: "forward", forwarded: true} = Repo.one!(Call)
    ...
    test "it should not forward the call if not all symptoms are positive", %{campaign: campaign, call: call} do
      ...
      assert %Call{current_step: "educational", forwarded: false} = Repo.one!(Call)
    ...
    test "it should leave the call as forwarded if it was", %{campaign: campaign, call: call} do
      ...
      assert %Call{current_step: "thanks", forwarded: true} = Repo.one!(Call)
    end
  end
